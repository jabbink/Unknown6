...
console.log("Injected on "+ platform)
if (platform == 'darwin') {
  var sig = ''
  console.log('iOS')
  // iOS
...
co(function *() {
  ...
  var attach = '';
  if (device.name == 'iOS Device' || device.name == 'iPhone') {
    attach = 'Pokémon GO';
  } else {
  ...
  const script = yield process.createScript(theScript)
    ...
    script.events.listen('message', (message, buf) => {
      if (message.payload.direction == 'in') {
        counter++;
      }
      // Store the output; eg. route through protoc --decode_raw
      fs.writeFile(message.payload.direction + '_dump_'+ counter + '.bin', buf, (err) => {
        if (err) {
          throw err;
        }
      });
